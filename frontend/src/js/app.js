...
function setSessionState(nextState) {
  if (sessionState === nextState) return;
  if (APP_CONFIG.debug) console.debug(`[scanner] ${sessionState} -> ${nextState}`);
...
function detectSupport() {
  ...
  if (!window.isSecureContext && !isLocalhost) {
    return { supported: false, reason: "insecure-context", message: "Open this scanner from HTTPS or localhost." };
  }
  if (!navigator.mediaDevices?.getUserMedia) {
    return { supported: false, reason: "camera-api-unavailable", message: "This browser cannot access the camera." };
  }
  if (!hasWebGLSupport()) {
    return { supported: false, reason: "webgl-unavailable", message: "This device or browser cannot render AR." };
  }
  return { supported: true, reason: "supported", message: "Ready to scan" };
}
...
async function requestCameraAccess() {
  try {
    // 30fps cap + 720p to reduce CPU load and latency
    ...
    if (name === "notallowederror" || message.includes("permission")) return { granted: false, reason: "denied", error };
    if (message.includes("insecure")) return { granted: false, reason: "insecure-context", error };
...
function checkCameraPermission() {
  ...
  if (typeof navigator.permissions !== "undefined" && navigator.permissions.query) {
    navigator.permissions
      ...
      .catch(() => {
        if (isMobileDevice()) showCameraPermissionPopup();
        else { setStatus("Ready to scan"); setStartEnabled(true); }
      });
  } else {
    if (isMobileDevice()) showCameraPermissionPopup();
    else { setStatus("Ready to scan"); setStartEnabled(true); }
...
function formatErrorDetails(error) {
  ...
  for (const key of Object.keys(error)) {
    if (key !== "name" && key !== "message" && key !== "stack") {
      ...
      } catch { parts.push(`${key}: [unable to serialize]`); }
    }
  }
  return parts.join("\n");
}
...
async function startScanner() {
  if (scannerStarted || sessionState === "starting") return;
  clearError();
  ...
  try {
    ...
    measurePerformance("asset preflight", "asset-preflight-start", "asset-preflight-end");
    markPerformance("session-create-start");
    session = createARSession(
      { ...APP_CONFIG, targetSrc: manifest.targetSrc, experiences: manifest.experiences },
      { onTargetFound: handleTargetFound, onTargetLost: handleTargetLost },
    );
    markPerformance("session-create-end");
    measurePerformance("session create", "session-create-start", "session-create-end");
    markPerformance("overlay-create-start");
    createExperienceOverlays();
    markPerformance("overlay-create-end");
    measurePerformance("overlay create", "overlay-create-start", "overlay-create-end");
...
async function handleCameraAllow(event) {
  ...
  if (result.granted) {
    ...
    if (result.reason === "denied") {
      ...
      showDetailedError("Camera access failed. Check browser settings and try again.", result.error);
    }
...
async function handleTargetFound(targetIndex) {
  ...
  if (videoPlane) {
    ...
    try {
      ...
      measurePerformance(`target ${targetIndex} video play`, `target-${targetIndex}-play-start`, `target-${targetIndex}-play-end`);
    } catch (error) {
...
function handleTargetLost(targetIndex) {
  ...
  targetState.lostTimer = window.setTimeout(() => {
    if (targetState.visible) return;
    const videoPlane = videoPlanes.get(targetIndex);
    if (videoPlane) {
      videoPlane.pause();
      videoPlane.mesh.visible = false;
    }
    if (activeTargetIndex === targetIndex) activeTargetIndex = null;
...
function findExperience(targetIndex) {
  return manifest?.experiences.find((experience) => experience.targetIndex === targetIndex);
}
...
function createExperienceOverlays() {
  ...
  for (const experience of manifest.experiences) {
    ...
    if (!anchor) throw new Error(`Missing anchor for targetIndex: ${experience.targetIndex}`);
    anchor.group.add(videoPlane.mesh);
    videoPlanes.set(experience.targetIndex, videoPlane);
    targetStates.set(experience.targetIndex, { visible: false, lostTimer: null });
  }
}

function getTargetState(targetIndex) {
  if (!targetStates.has(targetIndex)) targetStates.set(targetIndex, { visible: false, lostTimer: null });
  return targetStates.get(targetIndex);
}

function clearLostTargetTimer(targetIndex) {
  const targetState = getTargetState(targetIndex);
  if (!targetState.lostTimer) return;
  window.clearTimeout(targetState.lostTimer);
  targetState.lostTimer = null;
}

function pauseAllVideos() {
  for (const videoPlane of videoPlanes.values()) videoPlane.pause();
}

function disposeScanner() {
  for (const targetIndex of targetStates.keys()) clearLostTargetTimer(targetIndex);
  for (const videoPlane of videoPlanes.values()) videoPlane.dispose();
  videoPlanes.clear();
  targetStates.clear();
...
function handleVisibilityChange() {
  if (document.hidden) { pauseAllVideos(); return; }
  if (scannerStarted && activeTargetIndex !== null) setStatus("Point camera at the card");
}

function handleBeforeUnload() { disposeScanner(); }
...
function classifyStartupError(error) {
  ...
  const name = String(error?.name ?? "").toLowerCase();
  if (!window.isSecureContext && window.location.hostname !== "localhost") return "insecure-context";
  if (name === "notallowederror" || message.includes("permission")) return "camera-denied";
  if (name === "notfounderror" || name === "devicesnotfounderror" || message.includes("no camera")) return "camera-unavailable";
  if (message.includes("webgl")) return "webgl-unavailable";
  if (message.includes("manifest") || message.includes("target") || message.includes("assets/") || message.includes("404") || message.includes("failed to fetch")) return "asset-load-failed";
  return "unknown";
}
...
function getPlaybackErrorMessage(error) {
  const name = String(error?.name ?? "").toLowerCase();
  const message = String(error?.message ?? "").toLowerCase();
  if (name === "notallowederror" || message.includes("user gesture")) return "Tap Replay to start the video.";
  if (message.includes("format") || message.includes("decode")) return "This video format is not supported on this device.";
...
async function toggleMute() {
  ...
  for (const videoPlane of videoPlanes.values()) videoPlane.setMuted(isMuted);
}

async function replayVideo() {
  const videoPlane = videoPlanes.get(activeTargetIndex);
  if (!videoPlane) { setStatus("Point camera at the card"); return; }
  try { await videoPlane.replay(); clearError(); setStatus("Playing"); }
